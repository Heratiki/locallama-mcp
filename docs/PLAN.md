...
>
> **Config reload decision (Issue #24, 2026-05-20).** Runtime config reload is now explicit-call only via `reload_config` (no file watcher or polling). Reload is atomic: parse `.env` -> validate full candidate config -> apply only hot-reloadable fields. On validation failure, reload is rejected and the existing runtime config remains unchanged. Restart-required fields (provider endpoints/keys, server bind config, cache/log paths) are intentionally not applied at runtime.
